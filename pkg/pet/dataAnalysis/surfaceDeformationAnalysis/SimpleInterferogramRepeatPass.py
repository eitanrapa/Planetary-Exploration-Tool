...
class SimpleInterferogramRepeatPass(pet.component,
    ...
    """
    Class that creates an interferogram from a repeat pass given an instrument and orbit
    """
    ...
    time_offset_first_acquisition.default = 0.0
    time_offset_first_acquisition.doc = "time offset to apply to first acquisition [s]"

    time_offset_second_acquisition = pet.properties.float()
    time_offset_second_acquisition.default = 0.0
    time_offset_second_acquisition.doc = "time offset to apply to first acquisition [s]"

    forward_data = None
    inverse_data = None

    @classmethod
    def from_file(cls, planet, instrument, campaign, deformation_map, forward_phase_file_name, inverse_phase_file_name):
        """
        Load the interferogram from an HDF5 file
        :param planet: Planet object
        :param instrument: Instrument object
        :param campaign: Campaign object
        :param deformation_map: Deformation map file
        ...
        obj = cls(name="igram" + str(np.random.rand()),
                  planet=planet, instrument=instrument, campaign=campaign, track=track, deformation_map=deformation_map,
    ...
    @classmethod
    def from_files(cls, planet, instrument, campaign, deformation_map, forward_file_list, inverse_file_list=None):
        """
    ...
    def create_forward_data_array(self, phases, displacements, corrs, nlooks, sigma_phase, sigma_height, sigma_disp,
                                  nesn, sigma0):
        """
        Create a xarray with the input data
        :param phases: forward phases of interferogram
        :param displacements: Displacements in the line of sight
        ...
        forward_phases_da = xr.DataArray(
            ...
            coords={
                ...
                "los_displacement": ("points", displacements),
    ...
    def create_inverse_data_array(self, phases_ref, los_displacements_ref, kz_k):
        """
        Create a xarray with the input data
        :param phases_ref: Reference phases
        :param los_displacement_ref: Recovered displacements in the line of sight
        ...
        inverse_phases_da = xr.DataArray(
            ...
            coords={
                ...
                "los_displacement_ref": ("points", los_displacements_ref),
                "kz_k": ("points", kz_k),
                "x": ("points", self.track.data["x"].values),
                "y": ("points", self.track.data["y"].values),
                "z": ("points", self.track.data["z"].values),
    ...
    def solve_insar(vars, phi, wavelength, p_m, p_s, v_m, rho, doppler):
        ...
        :param p_m: position with uncertainty of the first satellite position
        :param p_s: position with uncertainty of the second satellite position
        :param v_m: velocity of the first satellite position
        :param rho: distance to the first satellite position
    ...
    def get_satellite_positions(self):
        """
        Get the satellite positions and velocities for the two satellite positions
    ...
    def get_satellite_positions_knowledge(self, satellite_positions1, satellite_velocity1):
        """
        Get the satellite positions and velocities for the two satellite positions with knowledge uncertainties
    ...
    def get_perpendicular_baseline(satellite_positions1, satellite_positions2, lines_of_sight):
        ...
        :param satellite_positions1: Satellite positions of the first satellite position
        :param satellite_positions2: Satellite positions of the second satellite position
    ...
    def calculate_igram(self):
        ...
        ground_satellite_vectors = satellite_positions1 - positions
        ...
        4 * np.pi * (1 / self.instrument.wavelength) * los_displacements
        ...
        self.create_forward_data_array(phases=phases_forward, displacements=los_displacements,
    ...
    def get_igram_inversion(self, use_dem="exact", dem_file=None):
        ...
        los_displacements_ref = (phases_forward - phases_ref) / (4 * np.pi / self.instrument.wavelength)
        ...
        self.create_inverse_data_array(phases_ref=phases_ref, los_displacements_ref=los_displacements_ref, kz_k=kz_k)
    ...
    def visualize_interferogram(self, projection, fig=None, globe=None, ax=None, return_fig=False):
        ...
        plt.savefig(fname=projection.folder_path + '/' + 'interferogram_' + self.deformation_map.pyre_name + '_' +
                    str(self.campaign.body_id) + '_' + str(self.forward_data.attrs["start_time1"]) + '_' + str(
                    self.forward_data.attrs["end_time1"]) +
                    '_' + str(self.forward_data.attrs["start_time2"]) + '_' +
                    str(self.forward_data.attrs["start_time2"]) + '.png', format='png',
                    dpi=500)
    ...
    def visualize_displacements(self, projection, fig=None, globe=None, ax=None, return_fig=False):
        ...
        longitudes = self.forward_data["longitude"].values
        latitudes = self.forward_data["latitude"].values
        los_displacements = self.forward_data["los_displacement"].values
